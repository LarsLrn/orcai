...
export const listBlocks = authed.block.list.effect(function* ({
	input,
	context,
}) {
	const db = yield* DB;
	...
	if (input.filters?.botId) {
		whereConditions.push(eq(dbSchema.botBlock.botId, input.filters.botId));
	}
	...
	.effect(function* ({ input }) {
		...
		if (input.status === "draft") {
			const linkedReadyBots = yield* db
				...
				.innerJoin(dbSchema.bot, eq(dbSchema.bot.id, dbSchema.botBlock.botId))
				.where(
					and(
