	...
	.effect(function* ({ input }) {
		...
		if (chatMessages.length > 0) {
			...
			const validParentIds = parentIds.filter(
				(id): id is NonNullable<ChatMessage["parentMessageId"]> => id !== null,
			);
					...
					and(
						...
						or(
							validParentIds.length > 0
								? inArray(dbSchema.chatMessage.parentMessageId, validParentIds)
								: undefined,
							hasRoot
			...
			const siblingsByParent = new Map<ChatMessageId | null, typeof siblings>();
