...
test("deletion transactions roll back with their outbox and committed cleanup replays", async ({
	...
	try {
		await db.query(`DROP TRIGGER IF EXISTS e2e_outbox_fault ON authz_outbox;
   ...
   CREATE TRIGGER e2e_outbox_fault BEFORE INSERT ON authz_outbox FOR EACH ROW EXECUTE FUNCTION e2e_outbox_fault();`);
		const fault = async (target: string, mode: string) => {
		...
		const latest = async (target: string) =>
			(
				await db.query(
		...
		const invitation = (
			await untilAllowed(() =>
				admin.organizationInvitation.create({
		...
		const asViewer = (
			await untilAllowed(() =>
				api.as("admin", doomed).organizationInvitation.create({
					organizationId: doomed.id,
					role: "viewer",
					expiresAt: new Date(Date.now() + 86400000),
					items: [
		...
		await fault("none", "delay");
		await untilAllowed(() =>
			api.as("admin", doomed).organizationMember.delete({
				organizationId: doomed.id,
				refs: [
					{
		...
		const unrelated = await untilAllowed(() =>
		...
		await db.query(
			"DROP TRIGGER IF EXISTS e2e_outbox_fault ON authz_outbox; DROP FUNCTION IF EXISTS e2e_outbox_fault(); DROP TABLE IF EXISTS e2e_outbox_fault;",
		);
		await db.query(
